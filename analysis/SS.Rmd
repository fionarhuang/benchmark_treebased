...
We simulate a scenario (`SS`) that only some leaves on the tree have
differential abundance (signal) between groups. The data is simulated by
swapping proportions of two branches. (Leaves in orange are not really a branch. Here, to mention them easily, we say they are in the orange 'branch'). Leaves in the same signal branch have the same fold change. 
